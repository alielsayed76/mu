...
@app.on_message(
    filters.command(["وقف", "تخطي", "استئناف", "توقف", "انهاء"])
    & filters.group
)
@AdminRightsCheck
@checker
async def admins(_, message: Message):
    ...
    if not len(message.command) == 1:
        return await message.reply_text("امر خاطئ")
    if not await is_active_chat(message.chat.id):
        return await message.reply_text("مفيش حاجه شغاله ")
    chat_id = message.chat.id
    if message.command[0][1] == "a":
        if not await is_music_playing(message.chat.id):
            return await message.reply_text("احن واقفين اصلا")
        await music_off(chat_id)
        await pause_stream(chat_id)
        await message.reply_text(
            f"🎧 اللي وقف الاغاني {message.from_user.mention}..."
        )
    if message.command[0][1] == "e":
        if await is_music_playing(message.chat.id):
            return await message.reply_text("🌸 شغالين اصلا")
        await music_on(chat_id)
        await resume_stream(chat_id)
        await message.reply_text(
            f"🎧 اللي شغل تاني {message.from_user.mention}..."
        )
    if message.command[0][1] == "t" or message.command[0][1] == "n":
        ...
        await message.reply_text(
            f"🎧 اللي انهاها {message.from_user.mention}..."
        )
    if message.command[0][1] == "k":
        ...
        if Queues.is_empty(chat_id):
            ...
            await remove_active_video_chat(chat_id)
            await message.reply_text(
                "مفيش اغاني تانيه موجوده __ǫᴜᴇᴜᴇ__ \n\nسلام"
            )
            await stop_stream(chat_id)
            return
        else:
            ...
            if str(finxx) == "raw":
                ...
                final_output = await message.reply_photo(
                    ...
                    caption=f"<b>__اتخطينا الاغنيه__</b>\n\n🎥<b>__بدات الاغنيه:__</b> {title} \n⏳<b>__المده:__</b> {duration_min} \n👤<b>__اللي طلبها:__ </b> {mention}",
                )
                await start_timer(
                    videoid,
                    duration_min,
                ...
                if int(quality) == 1080:
                    ...
                    final_output = await message.reply_photo(
                        ...
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=(
                            f"<b>__اتخطينا الاغنيه__</b>\n\n👤**__اللي طلبها:__** {mention}"
                        ),
                    )
                    await mystic.delete()
                else:
                    ...
                    final_output = await message.reply_photo(
                        ...
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=(
                            f"<b>__اتخطينا الاغنيه__</b>\n\n🎥<b>__بتشتغل:__ </b> [{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n👤**__اللي طلبها:__** {mention}"
                        ),
                    )
                    await mystic.delete()
                    os.remove(thumb)
                ...
                mystic = await message.reply_text(
                    f"**{MUSIC_BOT_NAME} قائمة التشغيل**\n\n__بنحمل اللي بعدها__"
                )
                (
                    title,
                    duration_min,
                ...
                final_output = await message.reply_photo(
                    ...
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=(
                        f"<b>__اتخطينا التشغيل__</b>\n\n🎥<b>__اشتغلت:__ </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n⏳<b>__المده:__</b> {duration_min} ᴍɪɴs\n👤**__اللي طلبها:__** {mention}"
                    ),
                )
                os.remove(thumb)
                await start_timer(
